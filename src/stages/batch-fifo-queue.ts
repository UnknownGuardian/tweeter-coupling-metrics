...
/**
 * A Batch FIFO queue implementation.
 * 
 * 
 * 
 * USING THE POLL IMPLEMENTATION
 */
export class BatchFIFOServiceQueue implements ServiceQueue {
  public readonly items: Item[] = [];
  private workers: Worker[] = [];
  private capacity: number = 0;

  public recentBatches:number[] = []; // array of timestamps, which we cull > 10 seconds
  public maxBatchSize:number = 10; // 10 messages per batch
  public longPollWaitTime:number = 20_000; // 20 seconds
  ...
  constructor(capacity: number, numWorkers: number) {
    ...
    metronome.setInterval(() => {
        this.longPoll();
    }, this.longPollWaitTime);
  }

  /**
   * Send a batch onwards, if items in queue.
   * @returns 
   */
  longPoll():void {
